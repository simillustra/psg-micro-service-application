...
class orange_credit_poultry_appraisal_controller
{
    ...
    {
        $this->orange_credit_poultry_appraisal_model = new orange_credit_poultry_appraisal_model();
    }
    ...
    public function invoke_orange_credit_poultry_appraisal()
    {
        ...
        if (get('do') == 'viewall') {
            if (PAGINATION_TYPE == 'Normal') {
                $result = $this->orange_credit_poultry_appraisal_model->SelectAll($query_keys, RECORD_PER_PAGE);
                //Accept get url  e.g (index.php?id=1&cat=2...)
                $paging = pagination($this->orange_credit_poultry_appraisal_model->CountRow($query_keys), RECORD_PER_PAGE, '' . H_ADMIN . '&view=orange_credit_poultry_appraisal&do=viewall&bid=' . $user_id . '&range_start=' . $range_start . '&range_end=' . $range_end);
            } else {
                $result = $this->orange_credit_poultry_appraisal_model->SelectAll($query_keys);
                $number_of_entry = $this->orange_credit_poultry_appraisal_model->CountRow($query_keys);
            }
